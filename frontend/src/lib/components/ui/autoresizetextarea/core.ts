...
class ProxyTextareaElement {
  ...
  __onpointerup = (e: PointerEvent) => {
    ...
    if (this._probablyResizeHappen) {
      const newHeight = parseNumber(
        this._element!.style.getPropertyValue("height")
      );
      if (
        !Number.isNaN(newHeight) &&
        newHeight !== this._lastCalculatedHeight
      ) {
  ...
  __getComputedHeight() {
    const { boxSizing, paddingSize, borderSize } = getSizingData(
      this._sourceStyles!
    );
